  ...
  return (
    <div className="calendar">
      <div className="calendar-header">
        <h1 className="text-xl font-bold text-center ">Veckoschema</h1>
      ...
      <div className="calendar-body">
        <div className="grid grid-cols-7 gap-1">
          {daysOfWeek.map((day, i) => (
            <div key={i} className="text-center font-bold">{day}</div>
          ))}
        </div>
        {hoursOfDay.map((hour) => (
               <div key={hour} className="grid grid-cols-7 gap-1" ref={hour === new Date().getHours() ? currentHourRef : null}>
              ...
              {channelScheduleResults[i]?.data
                ...
                .map((schedule, index) => (
                    <div key={index} className="space-y-2">
                      <span className="block text-lg font-semibold">{schedule.title}</span>
